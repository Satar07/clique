...
async fn find_max_clique(
    ...
    // 首先找出所有节点并排序
    let mut nodes: Vec<usize> = data.edges.iter()
        .flat_map(|(u, v)| [*u, *v])
    ...
    // 创建节点索引映射
    let node_to_idx: std::collections::HashMap<usize, NodeIndex> = nodes.iter()
        .enumerate()
    ...
    let max_clique: Vec<usize> = max_clique_indices.iter()
